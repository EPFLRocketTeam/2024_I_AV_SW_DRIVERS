...
//done
uint8_t WIRE_TEENSY::requestFrom(uint8_t address, uint8_t length, uint8_t sendStop)
{
	IMXRT_LPI2C_t* port = (IMXRT_LPI2C_t*)portAddr;
...
//done
uint8_t WIRE_TEENSY::requestFrom(uint8_t addr, uint8_t qty, uint32_t iaddr, uint8_t n, uint8_t stop)
{
	if (n > 0) {
...
//done
void WIRE_TEENSY::setClock(uint32_t frequency)
{
	IMXRT_LPI2C_t* port = (IMXRT_LPI2C_t*)portAddr;
	port->MCR = 0;
	if (frequency < 400000) {
...
//done
size_t WIRE_TEENSY::write(const uint8_t *data, size_t quantity)
{
...
//done
uint8_t WIRE_TEENSY::endTransmission(uint8_t sendStop)
{
	IMXRT_LPI2C_t* port = (IMXRT_LPI2C_t*)portAddr;
...
size_t WIRE_TEENSY::write(uint8_t data)
{
	if (transmitting || slave_mode) {
...
void WIRE_TEENSY::begin(uint8_t address)
{
	IMXRT_LPI2C_t* port = (IMXRT_LPI2C_t*)portAddr;
	CCM_CSCDR2 = (CCM_CSCDR2 & ~CCM_CSCDR2_LPI2C_CLK_PODF(63)) | CCM_CSCDR2_LPI2C_CLK_SEL;
	hardware.clock_gate_register |= hardware.clock_gate_mask;
